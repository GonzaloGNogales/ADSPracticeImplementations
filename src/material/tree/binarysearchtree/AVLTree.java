...
public class AVLTree<E> implements BinarySearchTree<E> {
    ...
    public Iterable<Position<E>> findRange(E minValue, E maxValue) throws RuntimeException {
        ...
        List<Position<E>> rangeList = new LinkedList<>();
        AVLInfo<E> avlmin = new AVLInfo<>(minValue);
        AVLInfo<E> avlmax = new AVLInfo<>(maxValue);

        if (avlmin.compareTo(avlmax) > 0)
            throw new RuntimeException("Invalid range. (min>max)");

        for (Position<E> p: this) {
            AVLInfo<E> aux = (AVLInfo<E>) p;

            if ((aux.compareTo(avlmin) >= 0) && (aux.compareTo(avlmax) <= 0)) {
                rangeList.add(p);
            }
        }
